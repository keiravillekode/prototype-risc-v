...
def encode_stone(stone):
    return hex((stone[0] << 4) | stone[1])

def array_literal(dominoes):
    return str(list(map(encode_stone, dominoes))).replace("[", "{").replace("]", "}").replace("'", "")
...
def gen_func_body(prop, inp, expected):
    ...
    str_list = []
    if len(dominoes) > 0:
        str_list.append(f"const uint8_t dominoes[] = {array_literal(dominoes)};\n")
