...
| `a1`     | temporary    | byte    | mine character '*'                            |
...
| `t6`     | temporary    | integer | number of adjacent mines                      |
*/

/* extern void annotate(char *buffer, const char *minefield); */
annotate:
        move    a2, a1
        lb      t3, 0(a2)               /* read first byte of minefield */
        beqz    t3, return
...
next_column:
        ...
        move    t3, t2
        move    t2, t1
        beq     t3, t4, check_mine      /* no more columns? */

        addi    t2, t2, 1               /* next column */

        check_mine:
        add     a2, a6, t1              /* address of minefield square */
        lb      t3, 0(a2)
        beq     t3, a1, write_square    /* jump ahead if we have reached a mine */

        move    t6, zero                /* number of adjacent mines */
        sub     a4, a5, t4
...
adjacent_column:
        ...
        addi    t6, t6, 1               /* update mine count */
...
next_adjacent:
        ...
        addi    t3, t6, '0'             /* mine count, as aSCII digit */
